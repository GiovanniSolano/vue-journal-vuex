...
describe('Pruebas en el Login Component', () => {
    ...
    test('Credenciales incorrectas, dispara el error de SWAL ', async() => {
    ...
    test('Redirigir a la ruta no-entry', async() => {
        ...
        const [textEmail, txtPassword] = wrapper.findAll('input')

        await textEmail.setValue('giovanni@gmail.com')
        await txtPassword.setValue('123456')

        await wrapper.find('form').trigger('submit')
        expect(store.dispatch).toHaveBeenCalledWith('auth/signInUser', {email: 'giovanni@gmail.com', password: '123456'})
