...
describe('Pruebas en useAuth', () => {
    ...
    test('createUser exitoso', async() => {
        ...
        const newUser = { name: 'Giovanni', email: 'gio@gmail.com'}
    ...
    test('createUser fallido, el email ya existe ', async() => {
        ...
        const newUser = { name: 'Giovanni', email: 'gio@gmail.com'}
        mockStore.dispatch.mockReturnValue({ok: false, message: 'EMAIL_EXIXTS'})

        const resp = await createUser(newUser)

        expect(mockStore.dispatch).toHaveBeenCalledWith('auth/createUser', newUser);
        expect(resp).toEqual({ok: false, message: 'EMAIL_EXIXTS'})
    ...
    test('Computed: authState, username', () => {
        ...
        expect(username.value).toBe('Giovanni')
